...
def zip_data(_ticker, symbol, online, **kwargs):
    balance_sheet_df, cash_flow_df, income_statement_df = read_data(_ticker, symbol, online, **kwargs)
...
@st.cache_data
def read_data(_ticker, symbol, online, **kwargs):
    ...
    if online:
        ...
        for statement in statements:
            try:
                ...
                statement_df = getattr(_ticker, statement)(**kwargs)
...
def get_statements_data(_ticker, symbol, online, **kwargs):
    try:
        balance_sheet_df, cash_flow_df, income_statement_df = read_data(_ticker, symbol, online, **kwargs)
...
def get_data(_ticker, symbol, online, **kwargs):
    df, statement_mappings = get_statements_data(_ticker, symbol, online, **kwargs)
...
def main(tickers, symbol, strings: dict, online: bool):
    ...
    frequency = st.sidebar.selectbox(label="Frequency", options=["Annual", "Quarter"])

    frequency = frequency[:1].lower()

    df, regular_variables, ratio_variables = get_data(tickers, symbol, online, trailing=False, frequency=frequency)
    ...
    with c2:
        st.session_state["selected_statements"] = st.multiselect(
                label = "Statements",
                options = (
                    np.sort(df["Statement"].dropna().astype(str).unique())
